...
public class Produttore2 implements Runnable{
	private int numero; 			/**@brief: numero casuale che il thread produttore genera ogni xms millisecondi */
	...
	private Buffer2 buffer;			/**@brief: istanza del buffer condivisa tra produttore e consumatore*/
	 ...
	 */
	public Produttore2 (Buffer2 buffer, final int maxNumero, final int minNumero, final int maxAttesa, final int minAttesa) {
		this.buffer = buffer;
	 ...
	 */
	private void produttoreBuffer2 () {
		 ...
		 */
		buffer.produttoreBuffer(new Dato (numero));
